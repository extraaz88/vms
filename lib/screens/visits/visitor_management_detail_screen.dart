...
class VisitorManagementDetailScreen extends StatefulWidget {
  ...
  const VisitorManagementDetailScreen({
    super.key,
    required this.visitId,
  });

  @override
  State<VisitorManagementDetailScreen> createState() => _VisitorManagementDetailScreenState();
}

class _VisitorManagementDetailScreenState extends State<VisitorManagementDetailScreen> {
  Visit? _visit;
  ...
  Future<void> _loadAreaDetails(double latitude, double longitude) async {
    try {
      ...
      final detailedAddress = await GeocodingService.getDetailedAddress(latitude, longitude);
      ...
      if (mounted) {
        setState(() {
          _areaName = '${latitude.toStringAsFixed(6)}, ${longitude.toStringAsFixed(6)}';
          _isLoadingArea = false;
        });
      }
    }
        ...
        children: [
          ...
          if (_isLoadingArea)
            ...
            if (_detailedAddress['street']?.isNotEmpty == true)
              _buildLocationInfo('Street', _detailedAddress['street'] ?? '', Icons.streetview),
            if (_detailedAddress['locality']?.isNotEmpty == true)
              _buildLocationInfo('Locality', _detailedAddress['locality'] ?? '', Icons.location_city),
            if (_detailedAddress['administrativeArea']?.isNotEmpty == true)
              _buildLocationInfo('State', _detailedAddress['administrativeArea'] ?? '', Icons.public),
            _buildLocationInfo('Coordinates', 
              ...
              Expanded(
                child: _buildTimingInfo(
                  ...
                  _visit!.checkOutTime != null 
                      ...
                      : _visit!.isActive ? 'Active' : 'Not completed',
                  Icons.logout,
                  _visit!.isActive ? AppTheme.warningColor : AppTheme.errorColor,
                ),
              ),
            ],
          ),
  ...
  Widget _buildTimingInfo(String label, String value, IconData icon, Color color) {
    return Container(
